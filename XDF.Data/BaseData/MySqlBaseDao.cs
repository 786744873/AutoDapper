...
namespace XDF.Data
{
    public class MySqlBaseDao<model> : IDisposable where model : class
    {
        public readonly MySqlDapperHelper dapperHelper;
        ...
        public MySqlBaseDao(string db)
        {
            dapperHelper = new MySqlDapperHelper(db);
        }
        #region CRUD
        public model Find(object Id)
        {
            var sql = new StringBuilder();
            sql.AppendFormat(MySqlFormat.Find, _Primary + "," + _Field, _Table, $" WHERE {_Primary}=@{_Primary}");
        ...
        {
            var sql = new StringBuilder();
            sql.AppendFormat(MySqlFormat.Filter, _Primary, _Field, _Table);
        ...
        public List<model> Filter(string where, object par, int limit,string order = "", string field = "*")
        {
            var sql = new StringBuilder();
            sql.Append("SELECT ");
            if (field.IsStringEmpty() || field == "*")
            ...
            if (!order.IsStringEmpty())
            {
                sql.AppendFormat(" ORDER BY {0} Limit {1}", order,limit);
            }
            else
            {
                sql.AppendFormat(" ORDER BY {0} DESC  Limit {1}", _Primary, limit);
            }
        ...
        {
            var sql = new StringBuilder();
            sql.AppendFormat(MySqlFormat.Del, _Table);
        ...
        public T Insert<T>(model info)
        {
            ...
            if (_IsIdentity) {
                strSql.AppendFormat(MySqlFormat.Insert, _Table, regex.Replace(_Field, ""), _FieldAt);
            }
            else {
                strSql.AppendFormat(MySqlFormat.Insert, _Table, _Primary + "," + regex.Replace(_Field, ""), "@" + _Primary + "," + _FieldAt);
            }
        ...
        {
            var strSql = new StringBuilder();
            strSql.AppendFormat(MySqlFormat.Update, _Table, _FieldUp);
        ...
        {
            var sql = new StringBuilder();
            sql.AppendFormat(MySqlFormat.Count, _Table);
            if (!where.IsStringEmpty()) {
                sql.AppendFormat(" WHERE {0} ", where);
            }
        ...
        {
            var sql = new StringBuilder();
            sql.AppendFormat(MySqlFormat.Single, field, _Table);
            if (!where.IsStringEmpty()) {
                sql.AppendFormat(" WHERE {0}", where);
            }
            return dapperHelper.Single<T>(sql.ToString(), par);
